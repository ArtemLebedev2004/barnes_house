...
export const dispatch = (action) => {
    ...
    if(action.type === 'updateDate'){
        ...
        updateDateGrid(state.value.date)
    }
}

const currentMonthDates = [] // will contain dates of current month
function currentDate() {
    const [ _, monthName, __, year ] = state.value.date.dateString.split(' ') // extracting month name and year
    const numberOfDays = getNumberOfDays(monthName, year) // recieving number of days
    const monthNumber = state.value.date.str.getMonth() + 1 // current month number 1 - 12
...
const arrangeGridAccordingToDays = currentMonthDates => {
    ...
    state.value = {...state.value, dateGrid: newDateGrid}
    return newDateGrid
}
currentDate()
state.value = {...state.value, dateGrid: currentMonthDates}
